...
class Trainer:
    ...
    def __init__(
        ...
        alpha_recon_reg: float = 1,
        alpha_recon_target: float = 1,
        alpha_kl: float = 1.,
        ...
        if (alpha_recon_reg < 0):
            raise ValueError('`alpha_recon_reg` must > 0.')
        if (alpha_recon_target < 0):
            raise ValueError('`alpha_recon_lode` must > 0.')
...
#       torch.use_deterministic_algorithms(True)
        ...
        ## update alpha and h initialization
        ...
        if mac_user:
            gpu_exist = torch.backends.mps.is_built()
        else:
            gpu_exist = torch.cuda.is_available()

        gpu = gpu_exist and use_gpu
        if gpu:
            if mac_user:
        ...
        alpha_unconstr_init = alpha_unconstr_init[target_index]
        log_h_int = log_h_int.to(self.device)
        ...
        self.model_kwargs = dict(
            ...
            alpha_recon_reg = alpha_recon_reg,
            alpha_recon_target = alpha_recon_target,
            alpha_kl = alpha_kl,
            W = W,
            corr_m = corr_m,
        ...
        self.model = regODE(**self.model_kwargs)
...
#        self.train_dl = DataLoader(self.train_dataset, batch_sampler = sampler)
    ...
    def train(self):
        ...
        if self.solver == "SGD":
            self.optimizer = torch.optim.SGD(params, lr=self.lr, momentum=self.wt_decay)
        if self.solver == "AdaBelief":
            self.optimizer = AdaBelief(params, lr=self.lr, eps=1e-16, betas=(0.9,0.999), weight_decouple = True, rectify = False)
        ...
        if self.scheduler_lr:
            ...
            if self.scheduler_model == "ReduceLR":
                self.scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(self.optimizer, mode='min',factor=0.9, patience=3, threshold=1e-09, threshold_mode='abs', cooldown=0, min_lr=0, eps=1e-09, verbose=False)
        ...
        with tqdm(total=self.nepoch, unit='epoch') as t:
            for tepoch in range(t.total):
                if self.scheduler_lr:
                   self.scheduler.step()
                train_loss, train_recon_loss_reg, train_recon_loss_target, train_kl_div = self._on_epoch_train(self.train_dl)
                val_loss, val_recon_loss_reg, val_recon_loss_target, val_kl_div = self._on_epoch_val(self.val_dl)
                ...
                self.log['train_recon_loss_reg'].append(train_recon_loss_reg)
                self.log['val_recon_loss_reg'].append(val_recon_loss_reg)
                self.log['train_recon_loss_target'].append(train_recon_loss_target)
                self.log['val_recon_loss_target'].append(val_recon_loss_target)
    ...
    def _on_epoch_train(self, DL) -> float:
        ...
        self.model.train()
        total_loss = total_recon_loss_reg = total_recon_loss_target = total_kl_div = .0
        ss = 0
        for X, Y in DL:
            self.optimizer.zero_grad()
            X = X.to(self.device)
            Y = Y.to(self.device)
            loss, recon_loss_reg, recon_loss_target, kl_div = self.model(X, self.ns)
            loss.backward()
            torch.nn.utils.clip_grad_norm_(self.model.parameters(), self.grad_clip)
            self.optimizer.step()

            total_loss += loss.item() * X.size(0)
            total_recon_loss_reg += recon_loss_reg.item() * X.size(0)
            total_recon_loss_target += recon_loss_target.item() * X.size(0)
            total_kl_div += kl_div.item() * X.size(0)

            ss += X.size(0)

        train_loss = total_loss/ss
        train_recon_loss_reg = total_recon_loss_reg/ss
        train_recon_loss_target = total_recon_loss_target/ss
        train_kl_div = total_kl_div/ss
        return train_loss, train_recon_loss_reg, train_recon_loss_target, train_kl_div
    ...
    def _on_epoch_val(self, DL) -> float:
        ...
        val_recon_loss_reg = total_recon_loss_reg/ss
        val_recon_loss_target = total_recon_loss_target/ss
        val_kl_div = total_kl_div/ss
        
        return val_loss, val_recon_loss_reg, val_recon_loss_target, val_kl_div
    ...
    def get_time(adata, self,n_samples = 1) -> np.ndarray:
        ...
        adata.layers["fit_t"] = T.values * scaling[np.newaxis, :]
    ...
    def get_vector_field(
        ...
        #if self.time_reverse:
        #    direction = -1
        beta = torch.clamp(F.softplus(model.v_encoder.beta_mean_unconstr), 0, 50)
        gamma = torch.clamp(F.softplus(model.v_encoder.gamma_mean_unconstr), 0, 50)
        velo = self.adata.layers["Mu"]*beta - self.adata.layers["Ms"]*gamma
