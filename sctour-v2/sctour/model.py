...
class regODE(nn.Module):
    ...
    def __init__(
        ...
        self.dt0 = torch.full((self.n_targets,), 1).to(self.device)

        ## generate variance
        self.scale_unconstr_regulators = torch.nn.Parameter(-1 * torch.ones(self.n_regulators))
        self.scale_unconstr_targets = torch.nn.Parameter(-1 * torch.ones(self.n_targets))

        ## generate gene specific maximum latent time
        self.max_time_unconstr = torch.nn.Parameter(7 + 0.5 * torch.randn(self.n_targets))
    ...
    def forward(self, x: torch.Tensor, n_samples=1) -> tuple:
        ...
        if self.global_time:
            ...
            t_max = torch.clamp(F.softplus(self.max_time_unconstr), 0, 20)
            T = self.decoder(z) * t_max
            T = T.T
            ...
            sol = solver.solve(to.InitialValueProblem(y0=x0, t_eval=t_eval), dt0=self.dt0)
        ...
        # generate variance
        scale_regulators = F.softplus(self.scale_unconstr_regulators)
        scale_targets = F.softplus(self.scale_unconstr_targets)
        
        scale_regulators = scale_regulators.expand(T.shape[1],self.n_regulators).sqrt()
        scale_targets = scale_targets.expand(T.shape[1],self.n_targets).sqrt()
        ...
        ## Using probability output to perform calculation
        #dist_u_regulators = Normal(emulation.T, scale_regulators)
        #dist_u_targets = Normal(pred_u.T, scale_targets)
        #dist_s_targets = Normal(pred_s.T, scale_targets)

        #recon_loss_regulator = -dist_u_regulators.log_prob(regulator_spliced.T)
        #recon_loss_target = -(dist_u_targets.log_prob(target_unspliced.T)+dist_s_targets.log_prob(target_spliced.T))
        #recon_loss_regulator = torch.mean(recon_loss_regulator.sum(dim=-1))
        #recon_loss_target = torch.mean(recon_loss_target.sum(dim=-1))

        ## compute KL divergence and z divergencec
