...
class MultitaskGPModel_latent(gpytorch.models.ApproximateGP):
    """ 
    Interpolate the regulator use the multitask variational gaussian process
    stochastic training, feasible to minibatch training (not like exact inference)
    need to define following parameters
    ...
    def __init__(
        ...
        # so that we learn a variational distribution for each task
        variational_distribution = gpytorch.variational.CholeskyVariationalDistribution(
            num_inducing_points = grid_size, batch_shape=torch.Size([num_latents])
        )
...
class velocity_encoder(nn.Module):
        ...
        if velocity_mode == "global":
            if interpolator == "GP":
                if self.pair_indices is not None:
                    self.multitaskGP_CRE = MultitaskGPModel(output_dim = n_cre, num_latents = 10).to(device)
                    self.likelihoodGP_CRE = gpytorch.likelihoods.MultitaskGaussianLikelihood(num_tasks=n_cre).to(device)
                    ## TODO: add device
                ## add TF expression
                self.multitaskGP_TF = MultitaskGPModel(output_dim = n_tf, num_latents = 10).to(device)
    ...
    def emulation_all(self,t: torch.Tensor = None):
        ...
        if self.mode == "global":
            ...
            if self.interpolator == "GP":
                #t_input = t.clone().detach()
                if self.pair_indices is not None:
                    emulate_CRE_m = self.multitaskGP_CRE(t_input)
                else:
                    emulate_CRE_m = None
                emulate_m = self.multitaskGP_TF(t_input)
    ...
    def forward(self,t,y):
        ...
        if self.mode == "global":
            if self.interpolator == "GP":
                t_input = t.clone().detach()
                emu_tf  = self.likelihoodGP_TF(self.multitaskGP_TF(t_input.view(-1))).mean
                ...
                if self.pair_indices is not None:
                    ...
                    emu_cre = self.likelihoodGP_CRE(self.multitaskGP_CRE(t_input.view(-1))).mean
            ...
            if self.interpolator == "RBF":
                ...
                if self.pair_indices is not None:
                    h = torch.exp(self.log_h_cre)
                    phi = torch.exp(self.log_phi_cre)
                    emu_cre = h * torch.exp(-phi*(t_input - self.tau_cre)**2) + self.o_cre
                    emu_cre_tf = emu_tf[self.pair_indices[:,0]] * emu_cre[self.pair_indices[:,1]]
            ...
            #alpha = torch.clamp(F.relu(alpha_unconstr),0,50)                  
...
class VELOVAE(BaseModuleClass):
    ...
    def __init__(
        ...
        #self.alpha_unconstr = torch.nn.Parameter(torch.randn(n_targets))
        ...
        #self.true_tensor = torch.ones(1, dtype=torch.bool)
        self.init_time = torch.tensor([0]).to(skeleton.device)
        self.x0 = torch.zeros(self.n_targets*2).unsqueeze(0).to(skeleton.device)
        self.dt0 = torch.full((self.x0.shape[0],), 1).to(skeleton.device)

        # saved kinetic parameter in velocity encoder module
        ## load require parameter for output velocity
        self.v_encoder.TF_func = None
    ...
    def loss(
        ...
        if self.interpolator == "GP":
            ...
            if emulation_cre is not None:
                ...
                recon_loss_reg_tf = -self.v_encoder.mll_TF(emulation,regulator_spliced)
                recon_loss_reg_cre = -self.v_encoder.mll_CRE(emulation_cre,accessibility)
                weight = sum(self.regulator_index) / self.v_encoder.n_cre
                recon_loss_reg = (recon_loss_reg_tf + weight * recon_loss_reg_cre) / 2
    ...
    def get_px(
        ...
        ind_t_input = ind_t.clone().detach()
        emulation, emulation_cre = self.v_encoder.emulation_all(ind_t_input.T)
    ...
    def sort_to_unsort_index(self, A, B):
        # Create a dictionary of the indices of each element in tensor B
        indices = {val.item(): i for i, val in enumerate(B)}
        
        # Use the indices dictionary to create the index vector that converts sorted A into unsorted B
        index_vector = [indices[val.item()] for val in A]
        
        # Convert the index vector back to a PyTorch tensor and return it
        return index_vector
    ...
    def _get_global_unspliced_spliced(
        ...
        t = t.ravel()
        if len(t)>1:
            t_eval, _ = torch.sort(t)
            index = self.sort_to_unsort_index(t,t_eval)
        ...
        t_eval = torch.cat((self.init_time,t_eval))
        ...
        pred_x = sol.ys[0,:,:]
        unspliced = pred_x[1:,:self.n_targets]
        spliced = pred_x[1:,self.n_targets:]
        
        if len(t)>1:
            unspliced = unspliced[index,:]
            spliced = spliced[index,:]
