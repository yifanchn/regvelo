...
class velocity_encoder(nn.Module):
    ...
    def forward(self,t, y):
        ...
        if self.RBF_interpolate:
            ...
            alpha_unconstr = torch.matmul(self.grn,s[self.regulator_index])
            #alpha_unconstr = self.fc1(s[self.regulator_index])
        ...
        alpha = F.sigmoid(alpha_unconstr)*torch.clamp(F.softplus(self.alpha_unconstr_max), 0, 50)
...
class VELOVAE(BaseModuleClass):
    ...
    def __init__(
        ...
        self.RBF_interpolate = RBF_interpolate
        self.lamba = lam
        ...
        self.switch_time_unconstr = torch.nn.Parameter(12 + 0.5 * torch.randn(n_targets))
        ...
        self.v_encoder.RBF_interpolate = self.RBF_interpolate
    ...
    def loss(
        ...
        local_loss = torch.mean(reconst_loss_target + weighted_kl_local)
